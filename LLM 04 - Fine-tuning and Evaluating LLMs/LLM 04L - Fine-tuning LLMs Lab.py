...
# TODO
tokenized_dataset=ds.map(tokenize,remove_columns=["instruction", "response", "context", "category"] )
#ds_to_token = tokenize(ds,1024)
#tokenized_dataset = ds.map(
    #imdb_to_tokens, batched=True, remove_columns=["instruction", #"response", "context", "category"]
#)
...
split_dataset = tokenized_dataset["train"].train_test_split(train_size=6000, shuffle=True)
trainer = Trainer(
    model,
    training_args,
